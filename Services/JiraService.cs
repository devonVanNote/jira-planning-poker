...
public class JiraService : IJiraService
{
    ...
    public string GetEffectiveJql(string? team = null)
    {
        ...
        if (!string.IsNullOrEmpty(team))
        {
            ...
            baseJql = $"\"{_teamField}\" = \"{teamGuid}\" AND issuetype IN (Story, Bug, Task) AND status NOT IN (\"Done\", \"Cancelled\", \"QA Pass\", \"QA Fail\", \"QA In Progress\", \"QA in Progress\", \"Missing Details\", \"User Stories\") AND summary !~ \"Regression Testing\" ORDER BY created DESC";
        }
        else
        {
            string configJql = _config["Jira:DefaultJql"] ?? "";
            baseJql = string.IsNullOrWhiteSpace(configJql)
                ? "issuetype IN (Story, Bug, Task) AND status NOT IN (\"Done\", \"Cancelled\", \"QA Pass\", \"QA Fail\", \"QA In Progress\", \"QA in Progress\", \"Missing Details\", \"User Stories\") AND summary !~ \"Regression Testing\" ORDER BY created DESC"
                : configJql;
        }
